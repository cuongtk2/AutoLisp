# SplitScreen — AutoCAD Screen Split Command
...
The `SPLITSCREEN` AutoLISP command automatically snaps **AutoCAD to the left half** of the screen and **File Explorer to the right half**, with a single command-line entry.

### Features

- Splits the screen 50/50, no manual dragging required.
- If an Explorer window is already open → **reuses it**, no extra window is spawned.
- If no Explorer window exists → opens a new one.
- No external libraries, no Python or other software required — uses PowerShell, built into Windows (7 SP1 and later).
- Runs completely hidden — no cmd/PowerShell window flashes during execution.
- A single `.lsp` file that generates and cleans up its own temp file.

### Installation

1. Download [`SplitScreen.lsp`](./SplitScreen.lsp).
2. In AutoCAD, run `APPLOAD` (or `NETLOAD` if needed), select `SplitScreen.lsp`, and load it.
3. (Optional) Add it to the **Startup Suite** in `APPLOAD` to auto-load every time AutoCAD starts.
...
AutoCAD will move to the left half of the screen; File Explorer will move (or open) to the right half.

### System Requirements

- Windows 7 SP1 or later (built-in PowerShell).
- AutoCAD (any version supporting AutoLISP).
- No administrator rights needed, no additional software required.

### How It Works

The command generates a temporary PowerShell file (`.ps1`) containing a small C# class (compiled at runtime via `Add-Type`) that calls Windows API functions (`user32.dll`):

- `EnumWindows` / `GetWindowText` — scans all open windows to locate AutoCAD.
- `GetClassName` — identifies genuine File Explorer windows (`CabinetWClass`), avoiding the hidden Desktop window (`Program Manager`), which also belongs to the `explorer.exe` process.
- `ShowWindow` / `SetWindowPos` — restores and repositions windows to the correct half of the screen.

The script is launched via the `WScript.Shell` COM object with a hidden window style (`0`), so no console window appears during execution. The temporary `.ps1` file is deleted automatically once the script finishes.

### Known Limitations

- AutoCAD window detection relies on the window title containing `autocad` or `autodesk` (case-insensitive). If your AutoCAD window title doesn't include these keywords (some special builds/localizations), detection may fail.
- Only supports a horizontal 2-way split (left/right), fixed at 50/50.
- Tested on single-monitor setups; multi-monitor systems may need further adjustment.

### License

Free to use, modify, and redistribute.

**[⬆ Back to top](#splitscreen--autocad-screen-split-command)**
...
**[⬆ Về đầu trang](#splitscreen--autocad-screen-split-command)**
